...
class ContrastiveLearningEncoder(nn.Module):
    ...
    With added classification head for direct classification during contrastive learning
    """

    def __init__(self, pretrained_model="MiniRBT-h256-pt", vector_size=256, num_classes=10):
        super(ContrastiveLearningEncoder, self).__init__()
        self.bert = BertModel.from_pretrained(pretrained_model)
        self.vector_size = vector_size
        # Add classification head for direct classification
        self.classifier = nn.Linear(vector_size, num_classes)

    def forward(self, input_ids, attention_mask):
        outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        # Get the [CLS] token representation
        embeddings = outputs.last_hidden_state[:, 0, :]
        return embeddings, self.classifier(embeddings)  # Return both embeddings and logits
    ...
    def encode_text(self, text, tokenizer, device):
        ...
        with torch.no_grad():
            embeddings, _ = self.forward(inputs["input_ids"], inputs["attention_mask"])
...
class AttentionMechanism(nn.Module):
    ...
    def forward(self, query_vector, neighbor_vectors):
        ...
        Returns:
            attention_weighted_vector: 邻居向量的加权和
        ...
        # 计算欧式距离 (使用负号让较近的距离有更高的值)
        distances = -torch.sqrt(torch.sum((query_expanded - neighbor_vectors) ** 2, dim=2) + 1e-8)

        # 应用softmax得到注意力权重
        attention_weights = F.softmax(distances, dim=1)  # 形状: (batch_size, k_neighbors)

        # 使用注意力权重加权邻居向量
        weighted_vectors = neighbor_vectors * attention_weights.unsqueeze(2)
        attention_weighted_vector = torch.sum(weighted_vectors, dim=1)  # 形状: (batch_size, vector_dim)
...
class TextClassifier:
    def __init__(self, num_classes,
        ...
        # Pass num_classes to the encoder for the classification head
        self.cl_encoder = ContrastiveLearningEncoder(pretrained_model, vector_size=256, num_classes=num_classes).to(
            device)
    ...
    def save_contrastive_model(self, save_path="bilstm-am-cl-nobert-best-contrastive_model.pt"):
        """
        保存对比学习模型及索引数据
        """
        # 创建保存目录(如果不存在)
    ...
    def load_contrastive_model(self, load_path="bilstm-am-cl-nobert-best-contrastive_model.pt"):
        """
        加载对比学习模型及索引数据
        """
        ...
        # 加载编码器参数
        self.cl_encoder.load_state_dict(state_dict['cl_encoder'],strict=False)
        ...
        if 'index_path' in state_dict and 'vectors_labels_path' in state_dict:
            ...
            if os.path.exists(index_path) and os.path.exists(vectors_labels_path):
                # 加载ANN索引
                self.annoy_index = AnnoyIndex(self.vector_size, 'euclidean')
    ...
    def supervised_contrastive_loss(self, features, labels, temperature=0.07):
        ...
        # 计算所有样本对之间的欧氏距离
        # 扩展维度以便计算成对距离
        x1 = features.unsqueeze(1)  # [batch_size, 1, vector_size]
        x2 = features.unsqueeze(0)  # [1, batch_size, vector_size]

        # 计算欧氏距离 (batch_size x batch_size 矩阵)
        # 距离越小表示越相似
        distances = torch.sqrt(torch.sum((x1 - x2) ** 2, dim=2) + 1e-8)

        # 将距离转换为相似度 (距离越小，相似度越高)
        # 使用负号将距离转换为相似度，再除以温度参数
    ...
    def train_contrastive(self, train_texts, train_labels, epochs=None, batch_size=32, gamma=0.5):
        """
        训练对比学习编码器
        使用欧氏距离作为度量标准
        ...
        for epoch in range(epochs):
            ...
            for batch in progress_bar:
                ...
                # 从编码器获取特征和logits
                features, logits = self.cl_encoder(input_ids, attention_mask)

                # 计算对比损失 - 使用修改后的基于欧氏距离的对比损失函数
    ...
    def _build_retrieval_index(self, train_texts, train_labels):
        ...
        # 对所有训练样本进行编码,使用进度条显示
        print("构建检索索引...")
        progress_bar = tqdm(enumerate(train_texts), total=len(train_texts), desc="编码文本")
        ...
        # 构建索引
        self.annoy_index.build(15)  # 使用15棵树以提高准确性
        print("检索索引构建成功")
    ...
    def _plot_training_losses(self, train_losses, val_losses):
        ...
        plt.plot(range(1, 11), val_losses, 'r-', label='Validation Loss')
        plt.title('Training and Validation Loss')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.xticks(range(1, 11))  # 显示所有epoch刻度
        plt.legend()
        plt.grid(True)
    ...
    def predict(self, texts):
        ...
        """
        # Set all models to eval mode for prediction
        self.model.eval()
        self.bilstm.eval()
        self.cl_encoder.eval()
        self.attention_mechanism.eval()
...
if __name__ == "__main__":
    ...
    # 定义对比学习模型保存路径
    contrastive_model_path = "./nobertmodels/bilstm-am-cl-nobert-best-contrastive_model.pt"
    ...
    classifier.train_classifier(
        ...
        epochs=10,
        batch_size=32,
        save_path="bilstm-am-cl-nobert-best_model.pth"
    )
    ...
    precision, recall, f1 = classifier.evaluate(
        ...
        model_path="bilstm-am-cl-nobert-best_model.pth"
    )